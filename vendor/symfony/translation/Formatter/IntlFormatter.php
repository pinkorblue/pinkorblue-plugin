...
class IntlFormatter implements IntlFormatterInterface
{
    ...
    public function formatIntl(string $message, string $locale, array $parameters = []): string
    {
        ...
        if (!$formatter = $this->cache[$locale][$message] ? $this->cache[$locale][$message] : null) {
            if (!($this->hasMessageFormatter ? $this->hasMessageFormatter : $this->hasMessageFormatter = class_exists(\MessageFormatter::class))) {
        ...
        foreach ($parameters as $key => $value) {
            if (\in_array($key[0] ? $key[0] : null, ['%', '{'], true)) {
